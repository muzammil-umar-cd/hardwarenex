    ...
    public function upload(Request $request)
    {
        ...
        if ($request->hasFile('aiz_file')) {
            ...
            if (isset($type[$upload->extension])) {
                ...
                $name = 'uploads/all'. $file_name . $request->file('aiz_file')->extension();
                
                // Upload the file
                $disk->put($name, file_get_contents($request->file('aiz_file')), 'public');
                ...
                $get_product_variation = ProductVariation::where('sku','=', $upload->file_original_name)->first();
                $get_brand = Brand::where('slug','=', $upload->file_original_name)->first();
                if($get_product_variation){
                    $get_product = Product::where('id','=',$get_product_variation->product_id)->first();
                    $get_product->photos = $upload->id;
                    $get_product->thumbnail_img = $upload->id;
                    $get_product->save();
                }elseif($get_brand){
                    $get_brand->logo = $upload->id;
                    $get_brand->save();
                }
